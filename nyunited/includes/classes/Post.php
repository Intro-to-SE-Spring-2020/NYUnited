...
class Post{
	...
	public function submitPost($body, $user_to) {
		...
		$body = mysqli_real_escape_string($this->con, $body);  // avoid any sql string injuctions
		$body = str_replace('\r\n', '\n', $body);
		$body = nl2br($body);     // replaces any new line with <br>
		$check_empty = preg_replace('/\s+/', '', $body);  // Delete all spaces

		if($check_empty != "")    // make sure the user doesn't post an empty post
		{
	...
	public function loadPostsFriends($data, $limit){
		...
		if (mysqli_num_rows($data_query) > 0) {
			...
			$num_iterations = 0; //Number of results checled (not necasserlily posted)
			$count = 1;
